...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    // UIWebView options
    self.inAppBrowserViewController.webView.scalesPageToFit = YES;//browserOptions.enableviewportscale;
    ...
    self.inAppBrowserViewController.preferredContentSize = CGSizeMake(150, 150);
...
- (void)createViews
{
    ...
    UITapGestureRecognizer *gesRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(close)];
...
//    self.view.backgroundColor = [UIColor clearColor];
//
////    self.view.superview.bounds = CGRectMake(0, 0, self.view.bounds.size.height, self.view.bounds.size.width);
//
//}
...
- (float) getStatusBarOffset {
    ...
    float statusBarOffset = IsAtLeastiOSVersion(@"7.0") ? MIN(statusBarFrame.size.width, 0) : 0.0;
